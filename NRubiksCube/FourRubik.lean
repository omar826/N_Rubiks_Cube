...
# Formalization of a 4x4x4 Rubik's Cube (FourRubik)

This file defines the state representation, basic moves, and visualization logic
for a 4x4x4 Rubik's Cube.

## Core Components:

1.  **State Representation (CubeState):**
    * Models the cube's state using permutations (Equiv.Perm) for the three
        piece types: corners (CornerSlot, Fin 8), edges (EdgeSlot, Fin 24),
        and centers (CenterSlot, Fin 24).
    * Tracks orientation using functions: corner_ori : CornerSlot → ZMod 3 and
        edge_ori : EdgeSlot → ZMod 2.
    * Edges are represented by 24 slots to distinguish the two type edge pieces called type A and B belonging to the same logical edge location.
    * Centers are represented by 24 slots, 4 for each face.
    * initialState defines the solved state (1 : CubeState).

2.  **Indexing Scheme (Indexing namespace):**
    * Provides constants (c_ufl, e_ur_a, z_f_ul, etc.) for referencing
        specific piece slots based on a standard face orientation convention
        (U=White, F=Red, R=Blue).
    * Defines lists of slots affected by each face/slice move (e.g.,
        r_face_corners, cr_slice_edges_a).
    * Includes logic for determining canonical piece colors/orientations
        (getCanonicalCornerColors, getCanonicalEdgeColors, etc.) and how
        orientation values affect the visible color (applyCornerOrientation,
        applyEdgeOrientation).
    * Defines orientation delta functions (r_move_corner_ori_delta,
        cr_move_edge_ori_delta, etc.) to specify how moves twist corners or
        flip edges.

3.  **Basic Moves (BasicMove inductive type):**
    * Defines the 12 fundamental moves: 6 outer face turns (R, L, U, D, F, B)
        and 6 inner slice turns (CR, CL, CU, CD, CF, CB).

4.  **Move Implementation (MoveImpl namespace):**
    * Defines the permutation effect of each BasicMove on corners, edges,
        and centers (e.g., r_move_corner_perm, cr_move_edge_perm).
    * Uses cube rotations (rotateCubeY) and permutation conjugation to
        derive the effects of L, D, B, CL, CD, CB moves from R, U, F, CR, CU, CF,
        reducing redundancy.
    * Implements the apply_move function, which takes a BasicMove and a
        CubeState and returns the resulting state by applying the corresponding
        permutations and orientation changes.

5.  **Visualization (Visual namespace):**
    * Defines StickerLocation to represent a specific sticker position on the
        unfolded cube (face, row, col).
    * Defines PieceStickerID to link a sticker location to the abstract piece
        slot and sticker index (0, 1, or 2 for corners; 0 or 1 for edges).
    * getSolvedPieceInfo: Maps a StickerLocation to the PieceStickerID that
        occupies that location in the solved state.
    * getStickerVisualColor: Calculates the color of a specific sticker at a
        given StickerLocation based on the current CubeState. It determines
        which piece is currently at that location, finds its original identity
        (using inverse permutations), gets its canonical colors, applies the current
        orientation value, and maps the resulting face orientation to a visual color.
    * stateToVisual: Converts an abstract CubeState into a Cube n_four
        structure suitable for printing via printUnfoldedCube from the
        NRubiksCube.visual library.

6.  *Examples and Utilities:*
    * Includes various view_... functions using #eval to display the cube
        state after specific moves or sequences (e.g., view_initial,
        view_after_R, view_corner_cycle).
    * Provides helper functions for move sequences (apply_move_list, inv_move,
        inv_move_list) and shorthand notations for moves (r, r', etc.).
